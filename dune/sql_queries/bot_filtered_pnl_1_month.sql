WITH
    filtered_traders AS (
        ...
        WHERE
            ...
            AND block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()  - INTERVAL '7' day
            AND trader_id NOT IN (
                SELECT
                    identifier
                FROM
                    dune.testnet3232.result_solana_bots
            )
        GROUP BY
            trader_id
    ),
    filtered_calls AS (
        ...
        WHERE
            call_block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()  - INTERVAL '7' day
    ),
    filtered_trades AS (
        SELECT
            t.trader_id,
        ...
        WHERE
            ...
            AND t.token_bought_mint_address in (
                SELECT
                    account_mint
                FROM
                    filtered_calls
    ...
    joined_data AS (
        ...
        FROM
            filtered_trades AS ft
            JOIN filtered_calls AS fc ON ft.token_bought_mint_address = fc.account_mint
            AND fc.account_user != ft.trader_id
            AND ft.block_time <= fc.call_block_time + INTERVAL '10' hour
    ),
    interesting_traders AS (
        SELECT
            trader_id
        FROM
            joined_data
    ...
    sell_tx AS (
        ...
        FROM
            ...
            and fst.block_time <= fc.call_block_time + INTERVAL '10' hour
    ),
    buy_summary as (
        SELECT
            account_mint,
    ...
    filtered_last_price_transactions as (
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY
                    ft.token_sold_mint_address
                ORDER BY
                    ft.block_time ASC
            ) AS rn
        FROM
            dex_solana.trades AS ft
            JOIN filtered_calls AS fc ON ft.token_sold_mint_address = fc.account_mint
        WHERE
            ft.block_time > fc.call_block_time + INTERVAL '10' hour
            AND ft.block_time < fc.call_block_time + INTERVAL '20' hour
    ),
    closest_transactions AS (
        SELECT
            account_mint,
            COALESCE(
                token_bought_amount / NULLIF(token_sold_amount, 0),
                0
            ) AS price
        FROM
            filtered_last_price_transactions
        WHERE
            rn = 1
    ),
    full_pnl_result as (
        SELECT
            ...
            GREATEST(
                COALESCE(bs.token_received, 0) - COALESCE(ss.token_spent, 0),
                0
            ) AS unsold_tokens,
            COALESCE(
                GREATEST(
            ...
            COALESCE(
                (
                    COALESCE(ss.sol_received, 0) + GREATEST(
                        (
                            COALESCE(bs.token_received, 0) - COALESCE(ss.token_spent, 0)
                        ) * COALESCE(ct.price, 0),
                        0
                    )
                ) - COALESCE(bs.sol_spent, 0),
                0
            ) AS profit_loss
        FROM
            buy_summary AS bs
            LEFT JOIN sell_summary AS ss ON ss.account_mint = bs.account_mint
            AND ss.trader_id = bs.trader_id
            LEFT JOIN closest_transactions AS ct ON ct.account_mint = bs.account_mint
    ),
    proftiable_data as (
        SELECT
            *,
...
SELECT
    ...
    COALESCE(CAST(SUM(win_hit) as DOUBLE) * 1.0 / COUNT(*), 0) AS win_hit_percentage
FROM
    proftiable_data
GROUP BY
    trader_id