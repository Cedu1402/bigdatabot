WITH tokens AS (SELECT account_mint
                ...
                WHERE call_block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()
                  AND call_block_time <= NOW() - INTERVAL '10' hour),
     unique_traders as (SELECT t.trader_id
                        FROM dex_solana.trades as t
                        WHERE t.token_sold_mint_address IN (SELECT *
                                                            FROM tokens)
                          AND block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()
                        GROUP BY t.trader_id
                        UNION ALL
                        SELECT t.trader_id
                        FROM dex_solana.trades as t
                        WHERE t.token_bought_mint_address IN (SELECT *
                                                              FROM tokens)
                          AND block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()
                        GROUP BY t.trader_id),
     filtered_trades AS (SELECT t.trader_id,
                                ...
                                'sell'                    AS trade_type
                         FROM dex_solana.trades AS t
                         WHERE t.trader_id IN (SELECT trader_id
                                               FROM unique_traders)
                           AND t.token_sold_mint_address IN (SELECT *
                                                             FROM tokens)
                           AND block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()
                         UNION ALL
                         SELECT t.trader_id,
                         ...
                         WHERE t.trader_id IN (SELECT trader_id
                                               FROM unique_traders)
                           AND t.token_bought_mint_address IN (SELECT *
                                                               FROM tokens)
                           AND block_time BETWEEN NOW() - INTERVAL '2' month AND NOW()),
     paired_trades AS (SELECT ft1.trader_id,
                       ...
                       FROM filtered_trades ft1
                                JOIN filtered_trades ft2 ON ft1.trader_id = ft2.trader_id
                           ...
                           AND date_diff('second', ft1.trade_time, ft2.trade_time) <=
                               2 -- Change to X minutes (e.g., 60*X for X minutes)
     )
SELECT trader_id,
