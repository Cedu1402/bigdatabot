...
from constants import INVESTMENT_AMOUNT, TOKEN_COlUMN, TOKEN_CLOSE_VOLUME_1M_QUERY, AGE_IN_MINUTES_COLUMN, \
    MARKET_CAP_USD, PRICE_COLUMN, TRADING_MINUTE_COLUMN
from evaluation.evaluation_plots import save_histogram
from evaluation.evaluation_statistic import print_statistics
...
def get_sell_time(token_price_data, token, data, label):
    try:
        if label:
            filtered_data = token_price_data[
                ...
                (token_price_data[PRICE_COLUMN] >= data[PRICE_COLUMN] * 2.3)]
        else:
            filtered_data = token_price_data[
                (token_price_data[TOKEN_COlUMN] == token) &
                (token_price_data[TRADING_MINUTE_COLUMN] > data[TRADING_MINUTE_COLUMN]) &
                (token_price_data[PRICE_COLUMN] >= data[PRICE_COLUMN] * 0.5)]
...
def run_simulation(val_x: pd.DataFrame, val_y: List[bool], prediction: List[bool]):
    ...
    for index, data in val_x.iterrows():
        ...
        if token not in token_bought:
            ...
            sell_time, max_sell_time = get_sell_time(token_price_data, token, data, val_y[index])
