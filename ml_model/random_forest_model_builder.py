...
from constants import BIN_AMOUNT_KEY, RANDOM_SEED, CRITERION, N_ESTIMATORS
...
def get_tree_hyperparameters(config: dict) -> dict:
    return {
        ...
        'classifier__' + N_ESTIMATORS: config.get(CRITERION, [200]),
    }
...
class RandomForestModelBuilder(SKLearnClassifierBuilder):
    ...
    def build_model(self, config: Dict = None):
        if config is not None:
            self.model = RandomForestClassifier(random_state=RANDOM_SEED, **config)
        else:
